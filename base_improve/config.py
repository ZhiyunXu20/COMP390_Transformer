"""base_improve：与 base_1 相同的数值超参与架构；仅输出路径 / W&B 项目名不同。"""
...
class Config:
    ...
    seed: int = 42
    num_workers: int = 6
    output_dir: str = "/root/autodl-tmp/base_improve/runs"
    project_name: str = "en-fr-attention-improve"
    use_wandb: bool = True
    wandb_run_name: str | None = None
    wandb_group: str | None = "base_improve"
    ...
    # 训练栈加速（不改变 d_model/batch/lr 等数值）
    use_torch_compile: bool = True

    # BLEU：语料中部分行英/法列相同或极似（数字等），易刷假高分；评估时跳过
    bleu_skip_identical_parallel: bool = True
    # 若 src 与 ref 解码串相似度 >= 该阈值（difflib），也跳过；None 表示关闭
