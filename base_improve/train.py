#!/usr/bin/env python3
"""base_improve：与 base_1 同架构与同数值超参；叠加 TF32、torch.compile、prefetch、fused AdamW。"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="base_improve EN-FR 训练（加速栈）")
    p.add_argument(
        "--attention",
        type=str,
        default="dot_product",
    ...
    p.add_argument("--max-steps", type=int, default=None, help="全局 batch 步数上限")
    p.add_argument("--val-every", type=int, default=None)
    p.add_argument("--no-attention-plots", action="store_true")
    p.add_argument(
        "--no-compile",
        action="store_true",
...
def main():
    ...
    compile_ok = False
    if device.type == "cuda" and getattr(cfg, "use_torch_compile", True):
        try:
            model = torch.compile(model)
            compile_ok = True
            print("base_improve: torch.compile 已启用", file=sys.stderr)
        except Exception as e:
            print(f"base_improve: torch.compile 跳过 ({e})", file=sys.stderr)
    ...
    stack_info = {
        "torch_compile": compile_ok,
        "tf32_matmul": bool(device.type == "cuda"),
        "tf32_cudnn": bool(device.type == "cuda"),
        "adamw_fused": fused_ok,
        "dataloader_prefetch_factor": loader_kw.get("prefetch_factor"),
    }
    ...
    if cfg.use_wandb and wandb is not None:
        wb_kw: dict = {
            ...
            "name": run_name,
            "config": {**cfg.__dict__, **stack_info},
        }
        if getattr(cfg, "wandb_group", None):
            wb_kw["group"] = cfg.wandb_group
        wandb.init(**wb_kw)
                ...
                if optimizer_step % cfg.val_every == 0:
                    ...
                    if bleu_diag:
                        msg += (
                            ...
                            f"跳过高相似={bleu_diag.get('bleu_skipped_high_similarity')}"
                        )
                    print(msg)
                    if cfg.use_wandb and wandb is not None:
    ...
    print("最终评估（metrics.json / W&B）…", file=sys.stderr)
    ...
    print(
        f"完成。checkpoint: {last} | final_val_loss={final_vloss:.4f} "
