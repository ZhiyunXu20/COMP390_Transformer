...
- **生成时仓库 HEAD（仅供参考）**：`157169a24b2c8ba12982d1f457c58914b7d1c905`（abbrev `157169a24b2c`）。
...
以下为 **`metrics.json`**（**35-run** 表）中出现的 **7** 个互异完整哈希（按 **git 历史从旧到新**；**`e6cd4aa7bb73`** 为 **A19** 训练记录，位于 **`8a9e118647` 之后**）：
...
在本机仓库中 **七枚 commit 均存在**；若在某次 clone 中 `cat-file` 失败，应标注 **verification not possible (commit not in local history)** 并跳过下文 blob 比对。

---

## Core file blob hash matrix

对下列 **核心文件** 执行 **`git rev-parse <commit>:<path>`**；并与 **当前 HEAD** 的 blob 对比。表中 **单元格** 为 **abbrev 12 位**；后缀 **`(same as HEAD)`** 表示该 commit 下文件内容与 **生成本文档时的 HEAD**（`157169a24b2c`）字节一致。
...
| Core file | `2c605436fe` | `97a3bfcb31` | `1ec4ccc52a` | `a750583efd` | `e09f93ad44` | `8a9e118647` | `e6cd4aa7bb73` | HEAD (`157169a24b2c`) |
|-----------|--------------|--------------|--------------|--------------|--------------|--------------|------------------|----------------------|
| `small_try/attention.py` | `19efa698384a` | `19efa698384a` | `caa2d2c3ce25` (same as HEAD) | `caa2d2c3ce25` (same as HEAD) | `caa2d2c3ce25` (same as HEAD) | `caa2d2c3ce25` (same as HEAD) | `caa2d2c3ce25` (same as HEAD) | `caa2d2c3ce25` |
| `small_try/model.py` | `d34880a57f23` (same as HEAD) | `d34880a57f23` (same as HEAD) | `d34880a57f23` (same as HEAD) | `d34880a57f23` (same as HEAD) | `d34880a57f23` (same as HEAD) | `d34880a57f23` (same as HEAD) | `d34880a57f23` (same as HEAD) | `d34880a57f23` |
| `small_try/train.py` | `eb23856347e0` | `eb23856347e0` | `19c6b9250fea` | `19c6b9250fea` | `fd5c25a8bf80` | `fd5c25a8bf80` | `73c49cf4a7e5` (same as HEAD) | `73c49cf4a7e5` |
| `small_head/train.py` | `3aa5fa4fdb49` | `3aa5fa4fdb49` | `ebdab4236b00` | `ebdab4236b00` | `609bba57df54` (same as HEAD) | `609bba57df54` (same as HEAD) | `609bba57df54` (same as HEAD) | `609bba57df54` |
| `small_swap/train.py` | `191a566b8042` | `191a566b8042` | `7e2c9b998167` | `7e2c9b998167` | `d9adfd130bb3` (same as HEAD) | `d9adfd130bb3` (same as HEAD) | `d9adfd130bb3` (same as HEAD) | `d9adfd130bb3` |
| `mt_eval.py` | `cbdb7c8b45f3` (same as HEAD) | `cbdb7c8b45f3` (same as HEAD) | `cbdb7c8b45f3` (same as HEAD) | `cbdb7c8b45f3` (same as HEAD) | `cbdb7c8b45f3` (same as HEAD) | `cbdb7c8b45f3` (same as HEAD) | `cbdb7c8b45f3` (same as HEAD) | `cbdb7c8b45f3` |
| `evaluate_test.py` | `cc04ac413e17` | `cc04ac413e17` | `0508056396ef` | `0508056396ef` | `0508056396ef` | `55a652b7f75f` (same as HEAD) | `55a652b7f75f` (same as HEAD) | `55a652b7f75f` |
| `scripts/significance_test.py` | `da63ef127781` | `9ab823ee6b1e` (same as HEAD) | `9ab823ee6b1e` (same as HEAD) | `9ab823ee6b1e` (same as HEAD) | `9ab823ee6b1e` (same as HEAD) | `9ab823ee6b1e` (same as HEAD) | `9ab823ee6b1e` (same as HEAD) | `9ab823ee6b1e` |
| `scripts/ablation_lib.py` | `9bf54d03bd0f` (same as HEAD) | `9bf54d03bd0f` (same as HEAD) | `9bf54d03bd0f` (same as HEAD) | `9bf54d03bd0f` (same as HEAD) | `9bf54d03bd0f` (same as HEAD) | `9bf54d03bd0f` (same as HEAD) | `9bf54d03bd0f` (same as HEAD) | `9bf54d03bd0f` |

**读表要点**：**并非**所有 commit 在 **`small_try/attention.py`** 上与当前 HEAD 一致——`2c605436fe` 与 `97a3bfcb31` 共享 **较旧** blob `19efa698384a`；自 **`1ec4ccc52a`** 起该文件与 `a750583efd` / **`e09f93ad44`** / **`8a9e118647`** / **`e6cd4aa7bb73`** / HEAD **一致**。**`small_try/train.py`** 在 **`a750583efd`→`e09f93ad44`** 变化一次；在 **`8a9e118647`→`e6cd4aa7bb73`** 再次前进到与 **当前 HEAD** 相同的 **`73c49cf4a7e5`**（**A19** 训练快照与此一致）。
...
3. **公平性审计（v4 墙）**  
   **`python scripts/check_variant_experiment_fairness.py --archive-audit`** 写出 **`results/variant_fairness_audit.md`**（**35** 行 = v4 **32** + **A19** **3**）：预期 **`swap_fr_dot` 仍为 FAIL**（语向 / tokenizer 刻意不同）；**不同 seed / `max_steps=1500`（相对主 run 3000）/ `learning_rate`（含 A19 的 `3e-3`）/ bf16 开关** 等相对 **`fast_dot` 参照**会标 **FAIL 或 WARN**（含 A15 **`local_window`** 探针），**非脚本缺陷**。

---

## Conclusion (v5 supplement — A19)

1. **`fast_add_lr3e3_s{1,2,3}`** 将归档墙扩至 **35** 个 **`metrics.json` 行**；训练记录 **`git_commit=e6cd4aa7bb73fa3888e9c6dbb917f3b7bac2fc01`**（git 历史上位于 **`8a9e118647` 之后**），引入第 **7** 枚 Distinct SHA（见上表 Order **7**）。
2. **核心文件**：**`e6cd4aa7bb73`** 与 **HEAD** 在 **`small_try/train.py`** / **`evaluate_test.py`** 等上与 blob 表一致；**A19** 与 **当前文档 HEAD** 在此意义上 **无新增 train/eval blob 分叉**（仍须 **`git_dirty=True`** caveat）。
3. **主表与审计**：**`scripts/summarize_attention_variants.py --multiseed-csv`** 现含 **`fast_add_lr3e3`** 行；公平性审计请始终使用 **`--archive-audit`** 以覆盖完整 **35** run。
...
# v5：A19（8a9 → e6cd）train 前进到与 HEAD 对齐的 blob
