...
- **生成时仓库 HEAD（仅供参考）**：`caf3d4ae6ded193d060f40e3814385cbaf4371fa`（abbrev `caf3d4a`）。
- **勿修改本文档引用的 JSON**：仅阅读 `metrics.json` / `resolved_config.json`；不改动任何 run 产物。

---

## v6 update（A23 — `safe_masked_softmax` / `var_local_window_a23_retrain`）
...
对下列 **核心文件** 执行 **`git rev-parse <commit>:<path>`**；并与 **当前 HEAD** 的 blob 对比。表中 **单元格** 为 **abbrev 12 位**；历史行中 **`(same as HEAD)`** 表示该 commit 下文件内容与 **当时** 生成本文档的 **HEAD** 字节一致（**v6** 以 **`caf3d4a`** 为准，见 **`bd1b53ebac69`** 列）。
...
| Core file | `2c605436fe` | `97a3bfcb31` | `1ec4ccc52a` | `a750583efd` | `e09f93ad44` | `8a9e118647` | `e6cd4aa7bb73` | `bd1b53ebac69` | HEAD (`caf3d4a`) |
|-----------|--------------|--------------|--------------|--------------|--------------|--------------|------------------|----------------|----------------------|
...
**读表要点**：**并非**所有 commit 在 **`small_try/attention.py`** 上与 **A23 前** 的最后一枚 **`e6cd4aa7bb73`** 一致——**`bd1b53ebac69`** 引入 **`safe_masked_softmax`** 与结构掩码协同，blob 从 **`caa2d2c3ce25` → `be0bb304288d`**。**`small_try/train.py`**、**`small_try/model.py`**、**`small_{head,swap}/train.py`**、**`evaluate_test.py`**、**`scripts/ablation_lib.py`** 在 **`e6cd`→`bd1b53`** 亦有前进（见上表）。**`mt_eval.py`** 与 **`scripts/significance_test.py`** 在此一步 **未变**。**`small_shared/attention_ops.py`** 仅自 **`bd1b53ebac69`** 起存在。**生成本文时的 HEAD**（`caf3d4a`）在列出的核心文件上与 **`bd1b53ebac69`** **字节一致**（其间为文档/审计类 commit）。v3 关于 **`2c`/`97a`** 上较旧 **`attention.py`** 的论述仍成立。
