...
Different runs in this archive were trained at different commits with `git_dirty=True` at the time of training. The **six** commits recorded across the **32** archived `metrics.json` rows are `2c605436fe` (main runs), `97a3bfcb31` (fast_dot multi-seed), `1ec4ccc52a` (fast_add multi-seed), `a750583efd` (single-seed `var_*` exploratory variants), **`e09f93ad44`** (A13 additive LR sweep: `lr_sweep_add_*`, `max_steps=1500`), and **`8a9e118647`** (A14 variant multi-seed + A15 `local_window` stability probes; `max_steps=1500` on those probes). **Core training/evaluation files are *not* byte-identical across all of these commits** (for example `small_try/attention.py` and `small_try/train.py` change between the older pair and the newer pair; see **§ v4 update** in the integrity doc for `a750`→`e09`→`8a9` deltas). A13, A14, and A15 add **two** commits and **16** runs beyond the original 16-run wall; **[docs/RUN_COMMIT_INTEGRITY.md](docs/RUN_COMMIT_INTEGRITY.md)** has the full **32-run** table, **six-commit** blob-hash matrix, diff summaries, and safe-vs-caveat notes. **`results/variant_fairness_audit.md`** lists the expanded fairness audit (32 `--runs`).
