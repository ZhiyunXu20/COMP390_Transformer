...
Different runs in this archive were trained at different commits with `git_dirty=True` at the time of training. The **seven** commits recorded across the **35** archived `metrics.json` rows (full **A17+A19** wall via `scripts/check_variant_experiment_fairness.py --archive-audit`) are `2c605436fe` (main runs), `97a3bfcb31` (fast_dot multi-seed), `1ec4ccc52a` (fast_add multi-seed), `a750583efd` (single-seed `var_*` exploratory variants), **`e09f93ad44`** (A13 additive LR sweep: `lr_sweep_add_*`, `max_steps=1500`), **`8a9e118647`** (A14 variant multi-seed + A15 `local_window` stability probes; `max_steps=1500` on those probes), and **`e6cd4aa7bb73`** (A19: `fast_add_lr3e3_s{1,2,3}`, `lr=3e-3`, `max_steps=3000`). **Core training/evaluation files are *not* byte-identical across all of these commits** (for example `small_try/attention.py` and `small_try/train.py` change between the older pair and the newer pair; see **§ v4 / v5 update** in the integrity doc for `a750`→`e09`→`8a9`→`e6cd` deltas). A13, A14, A15, and **A19** add runs beyond the original 16-run wall; **[docs/RUN_COMMIT_INTEGRITY.md](docs/RUN_COMMIT_INTEGRITY.md)** has the full **35-run** table, **seven-commit** blob-hash matrix, diff summaries, and safe-vs-caveat notes. **`results/variant_fairness_audit.md`** lists the expanded fairness audit (**`--archive-audit`**, 35 `--runs`).
