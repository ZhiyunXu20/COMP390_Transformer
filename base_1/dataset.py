"""平行语料加载、BOS/EOS、确定性 train/val 划分、collate。"""
...
class TabParallelDataset(Dataset):
    """
    Tab 分隔 EN \\t FR。
    val：约 val_ratio 的样本（idx % period == 0），与 Config.val_ratio 对齐。
    """
...
def tokenizer_special_ids(tgt_tok: Tokenizer) -> tuple[int, int, int]:
    """与现有 BPE 文件一致；用 id_to_token 校验，避免字符串字面量与 json 不一致。"""
