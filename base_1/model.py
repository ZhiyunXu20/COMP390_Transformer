...
- 输入嵌入 + 位置编码：`src_embed`/`tgt_embed` + `PositionalEncoding`（图中 Input Embedding、
  Positional Encoding）。
- 编码器：`n_layers` 个 `EncoderLayer`，每层含多头自注意力 + 前馈（`MultiHeadAttention` +
  `PositionwiseFFN`），对应图中 Encoder 重复块。
- 解码器：`DecoderLayer` 含掩码多头自注意力、对 memory 的多头交叉注意力、前馈，对应图中
  Decoder 的 Masked MHA、Encoder–Decoder MHA、FFN。
- 输出：`lm_head` 将 d_model 映射到目标词表（图中 Linear + softmax 的 logits 部分）。

实现细节与图中「Add & Norm」画法：本代码采用 **Pre-LN**（先 LayerNorm 再子层，再残差相加），
与论文原图常见的 Post-LN 画法顺序不同，但子模块集合与数据流与标准 Transformer 一致。

注意力核心由 `Config.attention_type` 在缩放点积与加性注意力之间切换。
"""
