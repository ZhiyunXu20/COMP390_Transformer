#!/usr/bin/env python3
"""small_head：单头 vs 多头对比训练；metrics.json 供 compare_head_runs.py 汇总。"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="small_head 单头实验（默认点积；加性需显式开启）")
    p.add_argument(
        "--attention",
        type=str,
        default="dot_product",
    ...
    p.add_argument(
        ...
        action="store_true",
        help="允许使用加性注意力（默认关闭；模型与 train 仍保留 additive 代码路径）",
    )
    p.add_argument(
        "--n-heads",
        type=int,
        default=None,
        help="覆盖 config.n_heads（例如单头实验传 1）",
    )
...
def main():
    ...
    if args.n_heads is not None:
        if cfg.d_model % args.n_heads != 0:
            print(
                f"错误: d_model={cfg.d_model} 必须能被 n_heads={args.n_heads} 整除",
                file=sys.stderr,
            )
            raise SystemExit(1)
